...
void eval(char *cmdline) {
    ...
    if (pid == 0) {  // Child process
        ...
        else { // If background job, print the job info
 ...
 */
void do_bgfg(char **argv) {
    // Check if the user has provided a job id or pid
    int isjid = 0;
    if (argv[1] == NULL) {
    ...
    // Check if the user has not provided a valid number for job id or pid
    if (isalpha(argv[1][0])) {
    ...
    // Determine if the job is specified by PID or JID
    if (argv[1][0] == '%') {
        isjid = 1;
    }
    ...
    if (strcmp(argv[0], "bg") == 0) {
        ...
        kill(-job->pid, SIGCONT);
        job->state = FG;
        // Wait on the job the same way as eval
        waitfg(job->pid);
        return;
    }
...
void waitfg(pid_t pid)
{
    // Sleep as long as the job with pid `pid` is the foreground job
...
void sigchld_handler(int sig) 
{
    ...
    while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0) {
        // If a child has terminated normally, delete the job associated with the child process
        if (WIFEXITED(status)) {
            deletejob(jobs, pid);
        }

        // If a child has been terminated, delete the job associated with the child process
        else if (WIFSIGNALED(status)) {
            printf("Job [%d] (%d) terminated by signal %d\n", pid2jid(pid), pid, WTERMSIG(status));
            deletejob(jobs, pid);
        }

        // If a child has been stopped, change the state of the job and print a message
        else if (WIFSTOPPED(status)) {
            getjobpid(jobs, pid)->state = ST;
            printf("Job [%d] (%d) stopped by signal %d\n", pid2jid(pid), status, WSTOPSIG(status));
        }
